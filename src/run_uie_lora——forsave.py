...
from compute_metrics import compute_metrics, compute_grouped_metrics,accuracy_score, f1_score
...
def main():
    ...
    data_cache_dir = gen_cache_path('/disk1/users/liying/AA_DataSharing/cache_Olora', data_args)
    ...
    # 尝试添加线性分类层,
    # if getattr(training_args, "use_probe", False):
    #     logger.debug(f'---add probe head---')
    #     num_probe_classes = training_args.probe_num_classes  # 例如 14
    #     hidden_size = model.config.hidden_size
    #     model.probe_head = Linear(hidden_size, num_probe_classes)
    #     for name, param in model.named_parameters():
    #         if not name.startswith("probe_head"):
    ...
    #         logger.info(f"Probe Head added successfully: {model.probe_head}")
    #     else:
    #         logger.warning("No probe_head found in model!")

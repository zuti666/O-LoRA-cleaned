    ...
    def forward(
        ...
        **kwargs,
        ...
        if not isinstance(peft_config, PromptLearningConfig):
            ...
            # ✨✨✨ 这里新增处理 probe_head ✨✨✨
            if hasattr(self, "probe_head") and getattr(self, "use_probe", True):

                if outputs.decoder_hidden_states is None:
                    raise ValueError("Decoder hidden states are required for probe head but not found. Make sure output_hidden_states=True.")



                # 提取 decoder 最后一层 hidden_states
                decoder_hidden_states = outputs.decoder_hidden_states[-1]  # (B, T, H)
                
                
                # 默认使用第一个 token 的特征 (CLS 风格)，也可以根据需要换成 mean/eos
                features = decoder_hidden_states[:, 0, :]  # (B, H)

                logits = self.probe_head(features)  # (B, num_classes)
